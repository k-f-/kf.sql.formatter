...
exports.autoAliasEverything = autoAliasEverything;
function autoAliasEverything(text, opts) {
...
function generateSemanticAlias(expr, counter) {
    if (/^max\(/i.test(expr))
        return `max_${extractArg(expr)}`;
    if (/^sum\(/i.test(expr))
        return `sum_${extractArg(expr)}`;
    ...
    if (/^case\b/i.test(expr))
        return `case_expr${counter}`;
    if (/^\w+\s*[\+\-\*\/]\s*\w+$/.test(expr))
        return `expr${counter}`;
    return `expr${counter}`;
}
function extractArg(expr) {
